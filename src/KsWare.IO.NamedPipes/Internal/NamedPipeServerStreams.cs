...
namespace KsWare.IO.NamedPipes.Internal {
	...
	public class NamedPipeServerStreams : NamedPipeStreams {
		...
		/// </summary>
		/// <param name="pipeName">Name of the pipe.</param>
		/// <param name="direction">The direction. (Only for API compatibility, always InOut)</param>
		...
		[SuppressMessage("Style", "IDE0060:Remove unused parameter", Justification = "parameter is part of public API")]
		...
		private void CreatePipes() {
			...
			_readPipe = new NamedPipeServerStream(PipeDirection.In, (_options & PipeOptions.Asynchronous) !=0, false, CreateNativePipe());

			// Create a write pipe for sending notifications to client.
			_writePipe = new NamedPipeServerStream($"{_pipeName}client", PipeDirection.Out, _maxNumberOfServerInstances, _transmissionMode, _options);

			RegisterPipes(_readPipe, _writePipe);
		}

		private SafePipeHandle CreateNativePipe() {
			// Create a read pipe for receiving notifications from the client.
			...
			// TODO PipeOptions.CurrentUserOnly netcore2.2+ net5.0+ netstandard2.1+ (.net 4.6.1)
			// TODO var openMode = unchecked((uint)PipeOptions.Asynchronous);

			var pipeMode = _transmissionMode == PipeTransmissionMode.Message ? PIPE_TYPE_MESSAGE : PIPE_TYPE_BYTE;
			...
			if (nativePipe.IsInvalid) throw new Win32Exception(error);
		...
		public void WaitForConnection() {
			...
			// the implemented workaround requires PipeOptions.Asynchronous to be specified in options
			...
			// workaround: in .net core are Tasks used and possibly the same AsyncWaitHandle is used
			if (r.AsyncWaitHandle == w.AsyncWaitHandle) r.AsyncWaitHandle.WaitOne();
			else WaitHandle.WaitAll(new[] {r.AsyncWaitHandle, w.AsyncWaitHandle});
